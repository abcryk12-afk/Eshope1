...
export default function CouponFormClient(props: CouponFormClientProps) {
  ...
  const [form, setForm] = useState<CouponFormState>(() => emptyState());

  const [categories, setCategories] = useState<CategoryItem[]>([]);
  const [products, setProducts] = useState<ProductListItem[]>([]);

  const couponId = props.mode === "edit" ? props.couponId : null;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const res = await fetch(`/api/admin/coupons/${encodeURIComponent(id)}`, { cache: "no-store" });
      ...
      if (!isRecord(json) || !isRecord(json.coupon)) {
        toast.error("Invalid response");
        setLoading(false);
        return;
      }

      const c = json.coupon as Record<string, unknown>;

      const startsAt = typeof c.startsAt === "string" ? c.startsAt : "";
      const expiresAt = typeof c.expiresAt === "string" ? c.expiresAt : "";

      const appliesTo = c.appliesTo === "categories" ? "categories" : c.appliesTo === "products" ? "products" : "all";
      const categoryIds = Array.isArray(c.categoryIds)
        ? (c.categoryIds as unknown[]).filter((x) => typeof x === "string") as string[]
        : [];
      const productIds = Array.isArray(c.productIds)
        ? (c.productIds as unknown[]).filter((x) => typeof x === "string") as string[]
        : [];
      ...
      setForm({
        ...
        minOrderAmount: c.minOrderAmount != null ? String(c.minOrderAmount) : "0",
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const usageLimit = form.usageLimit.trim() ? toInt(form.usageLimit) : undefined;
    if (usageLimit != null && (!Number.isFinite(usageLimit) || usageLimit < 1)) {
      toast.error("Usage limit must be an integer >= 1");
      return;
    }

    const usageLimitPerCustomer = form.usageLimitPerCustomer.trim()
      ? toInt(form.usageLimitPerCustomer)
      : undefined;
    if (usageLimitPerCustomer != null && (!Number.isFinite(usageLimitPerCustomer) || usageLimitPerCustomer < 1)) {
      toast.error("Per-customer usage limit must be an integer >= 1");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/admin/coupons/${encodeURIComponent(props.couponId)}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...payload,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Coupons</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">
            {props.mode === "create" ? "New coupon" : "Edit coupon"}
          </h1>
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Create coupons with expiry and usage limits.
          </p>
        </div>
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-3xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-900 dark:text-zinc-50">Code</label>
              <Input
                value={form.code}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-900 dark:text-zinc-50">Usage limit</label>
              <Input
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-zinc-900 dark:text-zinc-50">Usage limit per customer</label>
              <Input
                value={form.usageLimitPerCustomer}
                onChange={(e) => setForm((p) => ({ ...p, usageLimitPerCustomer: e.target.value }))}
                placeholder="(optional)"
              />
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-3xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <p className="mt-1 text-xs text-zinc-500">This will update the coupon immediately.</p>
